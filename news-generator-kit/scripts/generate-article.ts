/**
 * CLI: 生成新闻资讯 Markdown → content/news/YYYY-MM-DD-slug.md
 *
 *   npm run news:generate -- --topic "选题"
...
import type { BlogArticle, BlogCategory } from '../src/article-types.js'
import { BRAND, BRAND_KEYWORDS, FALLBACK_TOPICS } from '../src/brand.js'
import {
  BLOG_CATEGORIES,
  BLOG_CATEGORY_LABELS,
  BLOG_CATEGORY_UNSPLASH_QUERIES,
} from '../src/blog-categories.js'
import { pickIndexFromSeed, searchUnsplashCover } from '../src/unsplash.js'
import { isNewsDingTalkConfigured, notifyNewsPublished } from '../src/dingtalk.js'

const KIT_ROOT = resolve(fileURLToPath(new URL('.', import.meta.url)), '..')

function loadEnvFiles() {
  for (const name of ['.env.local', '.env']) {
    const path = join(KIT_ROOT, name)
...
function openaiClient(): OpenAI {
  ...
  if (!apiKey) {
    throw new Error('OPENAI_API_KEY is not set. Copy .env.example to .env and re-run.')
  }
...
async function discoverTopic(args: CliArgs): Promise<{
  ...
  const prompt = `你是 ${BRAND.name} 的资讯编辑。用你的联网搜索能力，找出**过去 14 天内**与以下方向相关的、值得写一篇深度文章的**一个**热点选题。
...
async function main() {
  ...
  console.log(`✓ Preview path: /blog/${slug}`)
